sap.ui.define([
	...
	return BaseController.extend("com.bom.sap.com.zbomapp.controller.Worklist", {
		...
		onBOMSerch: function () {
			...
			var oBomUsage = this.getView().byId("idBOMUsage").getValue().toString();
			...
			if (oMateriaNo.length === 0 || oPlant.length === 0) {
				...
				oFilter[2] = new sap.ui.model.Filter("BillOfMaterialVariantUsage", sap.ui.model.FilterOperator.EQ, oBomUsage);
				...
				oModel.read("/MaterialBOM", { // Get all valid material
					...
					success: function (oData, response) {
						...
						do {
							...
							j++;
							oFilter[j] = new sap.ui.model.Filter("Material", sap.ui.model.FilterOperator.EQ, oValidProdUsageMat[oValidMaterial]); //J101023862
							// Execute the oData for diffrent level -  Start

							oModel.read("/MaterialBOMItem", {
								filters: oFilter,
								urlParameters: {
									"$select": "BillOfMaterialVersion,Material,ComponentDescription,Plant,BillOfMaterialItemQuantity,BillOfMaterialCategory,IsAssembly,BillOfMaterialComponent,IsSubItem"
								},
								async: false,
								success: function (oData, response) {
		...
		onReset: function (oEvent) {
			...
			this.getView().byId("idBOMUsage").setValue("");
