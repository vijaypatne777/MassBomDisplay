					...
					success: function (oData, response) {
						...
						do {
							...
							oModel.read("/MaterialBOMItem", {
								filters: oFilter,
								urlParameters: {
									"$select": "BillOfMaterialVersion,Material,ComponentDescription,Plant,BillOfMaterialItemQuantity,BillOfMaterialCategory,IsAssembly,BillOfMaterialComponent,IsSubItem,BillOfMaterialItemNumber"
								},
								async: false,
								success: function (oData, response) {
		...
		handleSortStdConfirm: function (oEvent) {
			...
			if (oTableCount === 0) {
				...
				if (oSortObj === "Material") {
					...
					SORTKEY = "IsSubItem";
				}else if (oSortObj === "BOM Comp") {
					SORTKEY = "BillOfMaterialComponent";
				}
